...
## Header layout & mobile responsiveness (PRs #12–#14, #16–#17, #19)
...
**Takeaway for future layout work:** screenshot-test at actual mobile widths (375px/390px/430px covers the common range) before considering a responsive layout change done — several items in this section's history (#12, #17) were regressions/gaps that only surfaced in a real mobile screenshot, not in desktop-width review. For anything involving `position:sticky`, also verify the containing block is tall enough to hold the pin for the whole scroll range you expect (the #19 banner fix), and test at real scroll depth, not just a quick scroll-and-glance — a scripted test that fires `window.scrollTo` and screenshots too soon can catch a smooth-scroll animation mid-flight and look like a bug that isn't one (use `behavior:'instant'` or wait long enough, and cross-check the element's actual `getBoundingClientRect()` before trusting a screenshot alone).
...
- **"Automatically delete head branches" is now enabled** — the branches for PR #18 and PR #19 were both auto-deleted on merge (confirmed 2026-09-10 by their absence from the branch list right after merging), unlike every earlier PR. The 13 pre-existing branches below predate this and still need manual cleanup — the setting doesn't retroactively delete branches that already existed when it was turned on.
...
8. **Delete the 13 legacy merged `claude/`-prefixed branches** left over from PRs #10–#17 (and earlier), predating the auto-delete setting — see "Repo notes" below; an automated session can't delete them with its current git credentials.

## Repo notes

- `cowork-task-instructions.md` was rewritten from scratch in an earlier session.
- **13 legacy merged branches still need manual cleanup.** "Automatically delete head branches" is now enabled (see Outstanding action items — confirmed working for PRs #18/#19), but it doesn't retroactively delete branches that already existed when it was turned on. As of 2026-09-10, 13 merged `claude/`-prefixed feature branches from before that point are still present: `digest-email-header-border`, `fav-storage-race-fix`, `favorite-match-openers`, `header-size-tweak`, `notes-and-festival-data`, `project-notes-consolidated`, `project-notes-update-2`, `rfp-venue-url-wrap`, `riverfront-revival-venue-fix`, `star-favorite-prompt`, `star-modal-and-columns`, `utility-row-alignment-fix`, `utility-row-jump-links`. The git push credentials available to an automated session can push/create branches but get an HTTP 403 specifically on branch **deletion** (confirmed 2026-09-10 — a deliberate scope restriction, not a bug, and no GitHub MCP tool exists for it either) — so an automated session can't clean these up itself. Delete manually from the repo's Branches page.
