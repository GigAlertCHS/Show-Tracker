...
Consolidated reference for anyone (human or automated session) picking up work on this repo. Merges the GitHub `main` version (last updated 2026-09-10, through PR #17) with the Claude-project research/operations notes (last updated 2026-09-09), plus two same-day 2026-09-10 sessions: the first covering two Riverfront Park festival entries and two new standing policy decisions (Pops/jazz exclusion precedent, tiered-festival-pricing fallback); the second a full frontend UX pass (PRs #10–#17) covering favorite-artist matching against openers, storage-reliability fixes, a mobile layout overflow fix, a header utility-row reorg with new jump links, header sizing, a star-to-favorite-artist flow (including a per-artist picker for multi-artist/festival shows), and a sweep replacing every native `alert()`/`confirm()` with a consistent in-page modal. Replace all prior versions with this file.
...
## Header layout & mobile responsiveness (PRs #12–#14, #16–#17)

A same-day 2026-09-10 session made several changes to the page header and utility links, mostly driven by live mobile screenshots:

- **Riverfront Park venue-page mobile overflow (PR #12).** Riverfront Park's venue-site URL (`northcharleston.org/visitors/attractions_points_of_interest/riverfront_park/index.php`, ~97 chars) is far longer than any other venue's (next longest ~64 chars) and had no `overflow-wrap` rule — the unbroken `<a>` text forced the whole page wider than a mobile viewport instead of wrapping, squishing the visible layout to the left. Fixed with `overflow-wrap:anywhere` on both `.venue-site-link` and `.venue-address`.
- **Utility-row reorg (PR #13).** The row under the venue pills was reorganized into: a two-column row (**My Shows**/**Full List** jump links on the left — new — **Manage favorite artists**/**Suggest a venue** on the right — moved), then the sign-in/out control, then Last Updated. Plus "Back to top" links next to the Favorite Artists and Suggest a Venue section headings (the two sections furthest down the page), matching the existing footer back-to-top button's behavior.
- **Header sizing (PR #14).** `.b2-title`/`.b2-frame` sized up: exactly 10% on desktop (34px→37.4px font, 14px 24px→15.4px 26.4px padding). On mobile, a smaller bump (24px→27px font) was enough — the frame is an auto-width `inline-block` that already saturates to the full available container width whenever the title wraps to two lines (a shrink-to-fit quirk: once the one-line width exceeds the container, the box takes the full available width instead of its content width), so it was already effectively full-width before this change.
- **Sign-in/Last-Updated alignment regression (PR #16).** The utility-row reorg (#13) moved the sign-in button and Last Updated text out of a `.utility-col` (which had `align-items:flex-start`) into the bare row, which defaults to `align-items:stretch` — combined with `header.top`'s `text-align:center`, this centered both lines instead of left-aligning them like before. Fixed with `align-self:flex-start` on both.
- **Columns weren't actually side-by-side on mobile (PR #17).** The My Shows/Full List vs. Manage favorite artists/Suggest a venue split from #13 only rendered as two columns above a 640px breakpoint — below that (i.e. on an actual phone) it collapsed into one stacked column, so the left/right arrangement never showed up on mobile at all. `.utility-cols` now stays `row`+`space-between` at every screen width; confirmed via testing it holds with no wrapping down to 375px (iPhone SE).

**Takeaway for future layout work:** screenshot-test at actual mobile widths (375px/390px/430px covers the common range) before considering a responsive layout change done — two of the five items above (#12, #17) were regressions/gaps that only surfaced in a real mobile screenshot, not in desktop-width review.
...
8. **Delete the 13 merged `claude/`-prefixed branches** left over from PRs #10–#17 (and earlier) — see "Repo notes" below; an automated session can't delete them with its current git credentials.
9. **Enable "Automatically delete head branches"** (repo Settings → General → Pull Requests) so merged branches stop accumulating going forward.

## Repo notes

- `cowork-task-instructions.md` was rewritten from scratch in an earlier session.
- **Merged branches are not auto-deleted.** As of 2026-09-10, 13 merged `claude/`-prefixed feature branches (from PRs #10–#17 and earlier: `digest-email-header-border`, `fav-storage-race-fix`, `favorite-match-openers`, `header-size-tweak`, `notes-and-festival-data`, `project-notes-consolidated`, `project-notes-update-2`, `rfp-venue-url-wrap`, `riverfront-revival-venue-fix`, `star-favorite-prompt`, `star-modal-and-columns`, `utility-row-alignment-fix`, `utility-row-jump-links`) are still present in the repo. GitHub only deletes a PR's branch automatically if "Automatically delete head branches" is enabled under Settings → General → Pull Requests, which it isn't currently. The git push credentials available to an automated session can push/create branches but get an HTTP 403 specifically on branch **deletion** (confirmed 2026-09-10 — a deliberate scope restriction, not a bug, and no GitHub MCP tool exists for it either) — so an automated session can't clean these up itself. Delete manually from the repo's Branches page, or enable the auto-delete setting so future merges clean up on their own (recommended — see Outstanding action items).
