...
// These four helpers mirror the equivalent functions in the site's own index.html
// exactly (showId's field order, extractArtistName's split pattern, normalizeName's
// regex, and matchesFavorite's substring-match direction). Keeping them identical is
// what makes a show's id/artist match consistently between the site and this Worker —
// if one side's version of these functions ever changes, the other needs the same
// change, or personalization and unsubscribe-safe id matching will quietly diverge.
function showId(s) {
  return [s.v, s.d, s.b].join('|').toLowerCase().replace(/\s+/g, '_');
}
function extractArtistName(bandStr) {
  let s = bandStr.replace(/^new date:\s*/i, '');
  s = s.split(/\s+[—–-]\s+| \(/)[0];
  return s.trim();
}
...
function matchesFavorite(bandName, favorites) {
  const norm = normalizeName(bandName);
  return favorites.some(fav => {
    const favNorm = normalizeName(fav);
    return favNorm.length > 0 && norm.includes(favNorm);
  });
}
...
function buildDigestEmailHTML({ shows, venues, unsubscribeLink, siteUrl }) {
  ...
  // NOT personalized — every subscriber sees the same new-additions list, and is
  // pointed to the site to sign in for their own favorites, rather than the email
  // trying to reproduce personalized favorite content itself.
  ...
  const otherShows = upcoming.filter(s => !recentlyAddedIds.has(showId(s)));
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#1b1e2a; border:1px solid #2c3040; border-radius:8px; ${leftBorder}">
          <tr>
            ...
            <td style="padding:10px 14px;">
              <div style="font-size:11px; color:#9599ad; letter-spacing:0.5px;">${fmtShortDate(s.d)}</div>
  ...
  const signInCtaHtml = `
    <tr><td style="padding:20px 24px; text-align:center;">
      <div style="background-color:#1b1e2a; border:1px solid #2c3040; border-radius:8px; padding:16px;">
        <div style="font-size:13px; color:#c9cddb; margin-bottom:10px;">Sign in on the site to see your favorited artists and watchlist.</div>
  ...
  const emptyStateHtml = (!recentlyAddedShows.length && !otherShows.length) ? `
